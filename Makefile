...
# gameui.cpp). Rewritten only when the string actually changed, so `make`
# stays quiet; `make version` re-runs it on demand (e.g. after adding a
# tag). Override with `make version VERSION=1.0`. A fresh checkout gets
# the header generated by the no-prereq rule below (the first build).
.PHONY: version
version:
